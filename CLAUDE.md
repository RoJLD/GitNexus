...
(pinned to `v1.6.5`). Our deltas live in two places: the few files we
...
`--author` flags or `git -c user.email=...` to use the Alten work email.
...
| `patches/additive-files.diff` | New files we own (132 files, zero conflict risk). Regenerated by hand, see below. |
| `patches/inplace-edits.diff` | In-place edits to upstream files (21 files, the real conflict surface). Regenerated by hand, see below. |
...
1. **Regenerate `patches/additive-files.diff` and `patches/inplace-edits.diff`** if you edited anything
   inside `upstream/`. The diffs are what survive — un-staged upstream
...
6. **Commit at the gitnexus folder root** (this folder is the git repo).
   - Include all four: source files (top level only — `upstream/` is
     `.gitignore`-d), `patches/additive-files.diff` + `patches/inplace-edits.diff`, doc updates, and the
     new tests.
...
├── patches/
│   ├── additive-files.diff     New files we own (~132 files, zero conflict risk at bump)
│   ├── inplace-edits.diff      In-place edits to upstream files (21 files, real conflict surface)
