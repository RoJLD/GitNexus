...
try {
  ...
  // 2. tools/list — should list 13 tools (list_repos + 11 analytics + repo_by_id)
  const list = await send('tools/list');
  if (list.error) fail(`tools/list: ${list.error.message}`);
  const tools = list.result?.tools || [];
  if (tools.length !== 13) fail(`tools/list: expected 13 tools, got ${tools.length}`);
  for (const expected of [
    ...
    'gitnexus_similarity', 'gitnexus_repo_by_id',
  ]) {
