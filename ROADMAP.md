...
| **P2.2** | **Compléter les centralités (betweenness + eigenvector + sélecteur)** | Betweenness (Brandes) + eigenvector (power-iteration), pur-JS, ajoutés à `computeMetrics` ; overlay gagne un sélecteur de métrique (taille = degree/pagerank/betweenness/eigenvector, couleur reste = communauté) ; endpoint/MCP inchangés (champs traversent). [spec](docs/superpowers/specs/2026-06-03-graph-platform-p2-2-betweenness-eigenvector-design.md) | 📋 Spec écrite |
| **P2.3+** | **Reste de la boîte à outils (backlog)** | Structurel : **points d'articulation + ponts**, composantes connexes (faibles/fortes), k-core, clustering/transitivité, assortativité, densité, diamètre/excentricité. Communautés : Leiden général, label-propagation, Louvain multi-niveaux + résolution, comparaison vs Leiden index-time ASTKG. Centralités : closeness, Katz, harmonic. **Embeddings** : node2vec/DeepWalk (lourd). Sources : **ASTKG** (via `/api/graph`), **lenses**, graphes **merged/group** (+ cache + gros graphes : sampling/LoD/incrémental). Surfaces : heatmaps de centralité, filtre/isolation de communauté, panneau top-N, export CSV/JSON, couverture MCP complète. Détail : spec P2.1 §6 + P2.2 §6. Synergie : vision IA/Model-as-graph (dead-weights/hot-paths) + P3. | 💡 Backlog |
