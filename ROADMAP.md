...
| **Auto-updating Code Wiki** — doc toujours à jour | 🟡 **Partiel — existe upstream en CLI seulement.** `gitnexus wiki` pleinement implémenté côté core (`upstream/gitnexus/src/core/wiki/` : generator, graph-queries, llm-client, html-viewer ; CLI `upstream/gitnexus/src/cli/wiki.ts`), mais **CLI-only** → écrit `.gitnexus/wiki/index.html` ou un gist. **Zéro** route/panel wiki dans `gitnexus-web`. Pas "auto-updating". | 🛠️ **Design livré + en cours d'implémentation** : [`2026-05-28-code-wiki-web-ui-design.md`](docs/superpowers/specs/2026-05-28-code-wiki-web-ui-design.md) — panel iframe + génération via `wiki-worker.mjs` (conteneur server, spawn CLI) + auto-régen via cron watches. **Enhancements futurs** (out-of-scope v1) : (1) ré-render React natif au lieu de l'iframe (viewer thémé), (2) régen *staleness-based* (sur changement matériel du graph) en plus de l'intervalle, (3) config LLM provider in-UI au lieu de l'env-only, (4) gist publishing depuis l'UI (`--gist` existe en CLI). |
