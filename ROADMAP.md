...
| 75 | **Experts de périmètre câblés au MCP (Σ-PERIMETER-EXPERT V1, ELYSIUM Task 10 expdoc, 2026-07-22)** : deux outils exposent le contrat `/expert` de la gateway ELYSIUM — poser une question à un index sémantique scopé au lieu de grepper le repo. `elysium_list_experts` (`GET /expert`, découverte : nom, état d'index, nb de chunks, seuil de confiance calibré) et `elysium_ask_expert` (`POST /expert/<perimeter>`, `encodeURIComponent` sur le périmètre, corps `{question,mode,top_k}`). `mode="retrieve"` (défaut) rend des chunks cités (`path`+plage de lignes, score, confidence) ; `mode="answer"` ajoute une synthèse locale contrainte-citation qui **dégrade en chunks (`degraded:true`) plutôt que fabriquer** quand le modèle ou la confidence est insuffisant. **Point tranché à la mesure** : `mode="answer"` porte son propre timeout HTTP `ELYSIUM_EXPERT_ANSWER_TIMEOUT_MS` (défaut **180000**, ≥150 s exigé) car une synthèse 32B locale dépasse routinièrement le `GITNEXUS_TIMEOUT` de 30 s — sinon l'appel MCP avorte pendant que la gateway calcule encore ; le smoke prouve les **deux budgets distincts** (retrieve coupe au cap 250 ms sur un serveur 600 ms, answer survit sous 2000 ms). Vérif live e2e (2026-07-22) contre l'index calibré : `GET /expert` → `doctrine` present/8251 chunks/seuil 0.4827 ; `POST retrieve` « lock atomique sessions » → **SIGIL-561 top-3 avec citation `path:line`, Σ intact zéro mojibake**. Les 2 outils dégradent en stub documenté sans `INTER_GRAPH_URL` (Zero Masking) ; périmètre inconnu → erreur, pas résultat vide. Zéro changement dans `upstream/` → aucune régénération de patch. Côté ELYSIUM (repo hôte, branche `feat/expert-doctrine-v1`) : `.mcp.json` racine + `mcp_registry.json`. | `mcp-server/server.mjs` (+`elysium_ask_expert`, `elysium_list_experts`, `doPostCall`, `expertStub`, `ASKEXPERT_ANSWER_TIMEOUT_MS`), `mcp-server/smoke.mjs` (lock-step : count 39, stub, live, timeouts distincts, Σ round-trip), `mcp-server/README.md`, `INVENTORY.md` |
...
| **Divergence upstream** 🔎 **dry-run 2026-06-11** | Épinglé **v1.6.5** ; originel `abhigyanpatwari/gitnexus` `main` = **+257 commits** (dernier tag amont v1.6.8-rc.9). `node scripts/bump-upstream.mjs main` → **clean 138 / conflict 0 / fail 10**. Toute la surface **additive (~121 fichiers possédés) = 0 conflit** ; 7/17 inplace se réappliquent seuls ; **10 inplace à reprendre à la main** : `docker-server.mjs`, `package.json`+`lock`, `App.tsx`, `GraphCanvas.tsx`, `DropZone.tsx`, `Header.tsx`, `RepoAnalyzer.tsx`, `core/llm/agent.ts`, `hooks/useSigma.ts`. Rapport : `patches/bump-dry-run-main.md`. | Au prochain bump majeur | Le bump lui-même reste une décision (cibler un tag stable plutôt que `main` mouvant ; réappliquer les 10 inplace ; re-générer les patches + drift). |
...
| **P2.3-backlog** | **Directionnel + multi-niveaux + embeddings** | **Métriques directionnelles** (`?directed=1`) : in/out-degree, **HITS** (hubs/authorities), **composantes fortement connexes** (Tarjan itératif), betweenness directionnelle ; eigenvector/Katz/closeness/harmonic/communauté restent non-dirigées (HITS couvre le besoin directionnel). **Communauté multi-niveaux** (`?hierarchy=1`) : **Louvain complet** (agrégation super-nœuds récursive → dendrogramme) ; `communityPath` par nœud + résumé `hierarchy` par niveau ; `community` plat = niveau 0 (rétro-compat). **Embeddings spectraux** (`?embed=spectral&dims=k`) : Laplacian eigenmaps via power-iteration + déflation Gram–Schmidt (zéro-dép, réutilise le motif eigenvector ; node2vec rejeté — trop lourd en pur-JS) ; skippés au-delà du node-cap. Tout additif/opt-in (réponse byte-identique quand off) ; clé de cache + 2 outils MCP étendus. [spec](docs/superpowers/specs/2026-06-10-graph-platform-p2.3-backlog-design.md) · [plan](docs/superpowers/plans/2026-06-10-graph-platform-p2.3-backlog.md) | ✅ **Livré 2026-06-10** (moteur+routes+MCP+tests ; UI = follow-up) |
| **P2.3 (reste)** | **Au-delà du backlog ci-dessus** | Assortativité, diamètre/excentricité, comparaison vs Leiden index-time ASTKG, **Leiden** multi-niveaux (aujourd'hui Louvain seul), support arêtes pondérées, consommation UI des nouveaux champs (slider de niveau, panneau de similarité kNN, layout spectral), couverture MCP complète. Détail : spec P2.3-backlog §4 + spec P2.1 §6 + P2.2 §6 + P2.3.1 §6. | 💡 Backlog |
...
| 🔭 | **IA / Model as graph & as code** *(vision long-terme — débloquée par P2+P3 ✓)* | Un modèle IA *est* un graphe (couches / opérateurs / poids) → le **visualiser** dans gitnexus, faire de l'**observabilité** (quelles zones/couches/neurones s'activent à l'inférence) et de l'**optimisation** au même titre que du code. 3 phases : import « model graph » (HMM-export / ONNX / PyTorch-FX → nœuds = ops/layers/états, arêtes = tensors/transitions), observabilité (statique = théorie des graphes : nœuds morts/dead-weights, hot-paths/centralité ; dynamique = lens overlay activations runtime), optimisation « as code » (analytics + P2 sur le model-graph, diff de versions de modèle). Hors-scope déféré : PAS d'entraînement, PAS de serving/inférence, PAS un remplaçant de profiler/TensorBoard. Vision unifiée = un model-graph est *un import-template de plus* sur le SDK. Dépend de P2 + P3. Spec : [`docs/superpowers/specs/2026-06-03-ia-model-as-graph-vision-design.md`](docs/superpowers/specs/2026-06-03-ia-model-as-graph-vision-design.md). | 🚧 **P-IA.1 livré** |
| **P-IA.1** | **Import « model graph » (template SDK)** | Premier slice de la vision : template d'import `model-graph` (schéma dédié `ModelNode(id,type,label,layer)`/`ModelEdge(kind,weight)` — `type`=state/op/layer/observation, `kind`=transition/emission/tensor) + importeur `model-graph-json` lisant un `model-graph.json` curé (gitnexus-side, mirror de `research-graph` ; l'émetteur hmm_studio = follow-up). Render sur le canvas existant (couleur par type), interrogeable par la boîte à outils P2 (incl. directionnel/SCC) + les vues P3. **Zéro nouvelle architecture** (sidecar/routes/MCP template-driven inchangés). Fixture HMM synthétique + tests unit/registre verts + build web (tsc) vert ; round-trip d'intégration sidecar-gated (code conforme, non-exécuté en live — la stack de dev tient les ports). [spec](docs/superpowers/specs/2026-06-10-ia-model-as-graph-p-ia-1-import-design.md) · [plan](docs/superpowers/plans/2026-06-10-ia-model-as-graph-p-ia-1-import.md) | ✅ **Livré 2026-06-10** |
| **P-IA.2 (statique)** | **Observabilité structurelle (dead-weights + hot-paths)** | Capacité moteur **générique** (utilisable sur model-graph ET graphe de code) : `reachability` + `staticObservability` (BFS multi-source dirigée) ; sorties = puits structurels (out-degré 0, in-degré >0), entrées = sources ; **dead-weight** = nœud sans chemin dirigé vers une sortie (prunable). `?observability=1` (implique `directed`) ajoute par-nœud `reachesOutput`/`reachableFromInput`/`deadWeight` + `summary.deadWeightCount`/`outputCount`/`inputCount`/`observabilityDegenerate` ; additif/opt-in (byte-identique quand off), near-linéaire (tourne même cappé), clé de cache + 2 outils MCP étendus. **Hot-paths = réutilisent la centralité directionnelle existante** (betweenness/PageRank + heatmap — pas de nouveau champ). Highlight frontend des dead-nodes (rouge `#ef4444`, toggle « Observability ») via le 4ᵉ arg `opts` de l'adaptateur (mécanisme existant articulation/isolate). Hors-scope : tier *dynamique* (lens activations runtime), `role` explicite, actions de prune. 136 tests moteur + 22 MCP verts, build web (tsc) vert. [spec](docs/superpowers/specs/2026-06-10-ia-model-as-graph-p-ia-2-static-observability-design.md) · [plan](docs/superpowers/plans/2026-06-10-ia-model-as-graph-p-ia-2-static-observability.md) | ✅ **Livré 2026-06-10** |
| **P-IA.2 (dynamique)** | **Overlay d'activations (run d'inférence)** | Tier *dynamique* de l'observabilité : overlay sur le model-graph des **magnitudes d'activation par-nœud** (+ fréquences par-arête optionnelles) capturées d'un run d'inférence, rendu en heatmap. **Résout la question ouverte « format de capture » de la vision §6** : contrat `model-activations.json` (à côté du `model-graph.json` dans le source dir, clé sur les ids de l'importeur). `shapeActivations(doc)` pur dans le `-core` déjà-COPY'd ; route `GET /graph/activations/:name` (+ `?run=`) ajoutée au handler `handleGraphTemplatesRoute` **déjà câblé** (lecture pure, **zéro nouveau module / zéro changement Dockerfile.web** — comme `/graph/list`) ; outil MCP `gitnexus_graph_activations`. Overlay frontend « Activations » : nœuds heat-colorés par magnitude (réutilise `heatColor`) via `activationById`/`activationMax` ajoutés au 4ᵉ arg `opts` de l'adaptateur + toggle (gated aux model-graphs `?research=`). **Le producteur** (instrumentation runtime émettant le fichier) = follow-up cross-repo (comme l'émetteur model-graph.json). Hors-scope : edge-width, playback temporel, ingest Kùzu, échelle onnx. Fixture synthétique + 14 unit + 23 MCP verts, build web (tsc) vert. [spec](docs/superpowers/specs/2026-06-10-ia-model-as-graph-p-ia-2-dynamic-activations-design.md) · [plan](docs/superpowers/plans/2026-06-10-ia-model-as-graph-p-ia-2-dynamic-activations.md) | ✅ **Livré 2026-06-10** (tier observabilité COMPLET : statique + dynamique) |
| **P-IA.3** | **Optimisation/analyse « as code » + diff de versions de modèle** | Le « as code » de la vision : pointer les analytics existantes (P2 communautés=modules, centralité=hot-paths ; P-IA.2 dead-weights ; entropy/coupling=santé) sur le model-graph — **déjà disponible** (un model-graph est juste un graphe → endpoints P2/P-IA.2 inchangés, zéro nouveau code). La **seule pièce neuve = diff de versions de modèle** : `diffGraphs(a,b)` pur sur la forme universelle `{nodes,edges}` (nœuds ajoutés/retirés/**changés** type/label ; arêtes ajoutées/retirées par id `${from}->${kind}->${to}` ; `summary.drift`) dans le `-core` déjà-COPY'd ; route `GET /graph/diff?a=&b=` (rend les 2 graphes via `sidecarRender` + diff) ajoutée au handler **déjà câblé** (zéro nouveau module/Dockerfile) ; outil MCP `gitnexus_graph_diff`. **Distinct** du `computeGraphDiff` frontend (TS, forme KnowledgeGraph, presence-only) : backend, change-aware, MCP-accessible, host-native testé. Diff de poids d'arête + vue visuelle = déférés (le poids n'est pas surfacé par `sidecarRender`). 6 unit + 24 MCP verts. (+ fix infra : `maxBuffer` du check-patch-drift, le diff additif a dépassé 1 Mo.) [spec](docs/superpowers/specs/2026-06-10-ia-model-as-graph-p-ia-3-model-diff-design.md) · [plan](docs/superpowers/plans/2026-06-10-ia-model-as-graph-p-ia-3-model-diff.md) | ✅ **Livré 2026-06-10** (diff ; reste de P-IA.3 déjà couvert par P2+P-IA.2) |
| **Enabler** | **Render-prop passthrough + diff de poids d'arête** | Lève la limite récurrente « le poids n'est pas surfacé par `sidecarRender` » : `mapRenderRows` pur extrait dans `graphs-sidecar/render-map.mjs` (kuzu-free, host-testable) spread `...n`/`...r` puis champs calculés → **passthrough additif** de tout prop stocké (poids d'arête, `layer`, …) ; `kuzu-store.render` délègue (queries cypher inchangées). Premier consommateur : `diffGraphs` gagne un bucket **`edges.changed`** (delta de `weight` sur arêtes communes) + `summary.changedEdges`. Débloque (déférés) : activation edge-width + métriques pondérées. `graphs-sidecar/` = dir tracké (pas `upstream/`, pas de patch) ; **nécessite un rebuild du sidecar pour déployer**. 3 + 8 unit verts. [spec](docs/superpowers/specs/2026-06-10-graph-platform-render-prop-passthrough-design.md) · [plan](docs/superpowers/plans/2026-06-10-graph-platform-render-prop-passthrough.md) | ✅ **Livré 2026-06-10** |
| **P-IA.3 (vue diff)** | **Vue diff visuelle sur le canvas** | Le diff backend `/graph/diff` amené **sur le canvas** model/research : sélecteur « Compare ▾» (liste les autres graphes via `listGraphs`) → rend l'**union** de A+B colorée par statut (ajouté vert / retiré rouge / changé ambre / commun gris) + légende avec compteurs de drift. Libs pures graphology-free `gitnexus-web/src/lib/graph-diff-view.ts` (`buildDiffStatus` map id→statut, `unionResearchGraphs` **générique** type-transparent, `DIFF_VIEW_COLORS`) host-testées (comme `metrics-view.ts`) ; `opts.diffStatusById` ajouté à l'adaptateur (couleur par statut, réutilise la convention `DIFF_COLORS` du snapshot-diff) ; `getGraphDiff` client. Distinct du diff snapshot code-graph (`useAppState`) — c'est le pendant canvas research/model. Coloration d'arête + diff 3D = déférés. 4 unit verts + build web (tsc) vert (a attrapé un mismatch `DiffViewGraph`/`ResearchGraph` → corrigé en rendant `unionResearchGraphs` générique). [spec](docs/superpowers/specs/2026-06-10-ia-model-as-graph-p-ia-3-visual-diff-design.md) · [plan](docs/superpowers/plans/2026-06-10-ia-model-as-graph-p-ia-3-visual-diff.md) | ✅ **Livré 2026-06-10** |
| **P-IA.1 (onnx)** | **Convertisseur ONNX → model-graph** | La cible neuronale phare de la vision, sans nouvel importeur : un convertisseur produit un `model-graph.json` (importé par le template P-IA.1 existant). `tools/onnx-to-model-graph.mjs` (outil Node offline tracké, zéro-dép, comme `tools/academic-extract.mjs`) : transform pur `onnxGraphToModelGraph(onnxGraph,{name,maxNodes})` — ops→nœuds (`type:'op'`, label=opType), flux de tenseurs→arêtes (`kind:'tensor'`, producteur→consommateur, dedup) ; ids dédupliqués (`name` ou `opType#i`) ; garde `maxNodes` (échec bruyant, pas de troncature silencieuse). Le parse protobuf `.onnx`→JSON = **pré-étape Python offline documentée** (`MessageToDict`, hors-CI comme academic-extract) — la dép onnx n'est jamais dans le code servi. **Échelle/LoD (10⁴–10⁶ ops) = déférée** (la question ouverte de la vision ; le node-cap des métriques + la garde maxNodes la bornent). 6 unit verts ; zéro changement `upstream/` (tracké `tools/`, pas de patch/Dockerfile/build web). [spec](docs/superpowers/specs/2026-06-10-ia-model-as-graph-onnx-importer-design.md) · [plan](docs/superpowers/plans/2026-06-10-ia-model-as-graph-onnx-importer.md) | ✅ **Livré 2026-06-10** (transform+CLI ; LoD grande-échelle = LoD v1 ci-dessous) |
| **Render LoD v1** | **Niveau-de-détail au rendu (node-cap + top-N)** | La garde **côté rendu** qui manquait pour les gros graphes (vrai modèle ONNX) : lib pure graphology-free `gitnexus-web/src/lib/graph-lod.ts` (`pruneForRender(graph,{maxNodes,by})` host-testée) — au-delà de `LOD_MAX_NODES` (1500), garde les top-N nœuds par degré (tie-break id asc) + les arêtes entre eux, sinon no-op ; générique/type-transparent. GraphCanvas l'applique automatiquement avant le rendu (sur le graphe research/model ET l'union de diff) + bannière « LoD · showing N of M nodes (top by degree) » (`data-testid=lod-banner`) — **jamais de troncature silencieuse**. En-dessous du seuil : byte-identique. 5 unit verts + build web (tsc) vert. **Collapse op-group/communauté (LoD riche) + LoD 3D = déférés.** [spec](docs/superpowers/specs/2026-06-10-graph-platform-render-lod-design.md) · [plan](docs/superpowers/plans/2026-06-10-graph-platform-render-lod.md) | ✅ **Livré 2026-06-10** |

### 💡 Backlog Graph Platform / IA (enregistré 2026-06-11)

- **A.1 — Encaissement UI du moteur** ✅ **Livré 2026-06-11** : surface dans l'UI ce que le moteur produisait en API/MCP-only. **(1)** Métriques **directionnelles** dans le sélecteur de taille (toggle `?directed=1` → optgroup in/out-degree + HITS hubs/authorities). **(2)** **Slider de niveau de communauté** (toggle `?hierarchy=1` → `communityPath[level]` via l'opt adaptateur `communityOverrideById`). **(3)** **Layout spectral** (option du sélecteur de layout → `?embed=spectral` → `spectralLayout(embedding)` positions précalculées). **(4)** **Panneau de similarité kNN** (`nearestNeighbors` cosinus sur l'embedding → top-8 du nœud sélectionné, listés + surlignés via l'opt `knnIds`). Libs pures graphology-free `gitnexus-web/src/lib/embedding-tools.ts` host-testées (4 unit) ; client `MetricsOpts`/`GraphMetricNode`/`SizeMetricAny` étendus ; adaptateur + GraphCanvas câblés ; tout additif (byte-identique quand off) ; build web (tsc) vert. **Edge-weight-aware overlays (largeur d'arête activation/poids) = follow-up** (débloqué par le passthrough déployé). [spec](docs/superpowers/specs/2026-06-11-graph-platform-a1-ui-encashment-design.md) · [plan](docs/superpowers/plans/2026-06-11-graph-platform-a1-ui-encashment.md)
- **LoD riche** : collapse **op-group / communauté** (le vrai LoD, au-delà du prune top-N v1) + **LoD 3D**. 💡 Backlog.
- **Follow-ups IA** : export **NHMM / émissions continues** + **pytorch-fx** ; **merger le PR HMMstudio** (`feat/model-graph-export` → main). 💡 Backlog.
- **Template Library** : **zettelkasten**, **crypto** (Experiment.Crypto). 💡 Backlog.

> **Ordre convenu (2026-06-11)** : **A.1 (en cours)** → puis **consolidation UI horizontale** (~10 → 15+ panels : regroupement Health / Social / Cross-repo) + **observabilité produit** (smoke harness + `/metrics`) → enfin **divergence upstream** (+275 commits derrière `main` upstream — dérisquer le prochain bump). (Les 3 derniers sont déjà listés dans « Refactos structurels à surveiller » ci-dessus ; cette note fige la séquence.)
