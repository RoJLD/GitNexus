...
| **P2** | **Boîte à outils théorie-des-graphes** | Centralités, communautés, chemins/cycles, points d'articulation, embeddings — exposés par graphe (endpoints + MCP + overlays). **Décomposé** en P2.1 + P2.2 + P2.3.1 (livrés) + P2.3.2/3 (backlog). | 🚧 P2.1·P2.2·P2.3.1 livrés · P2.3.2/3 backlog |
...
| **P2.3.2 (B)** | **ASTKG comme source de métriques** | Faire tourner `computeMetrics` sur le **graphe de code** (ASTKG via `/api/graph` sur le canal interne `GITNEXUS_API`, comme le lens `imports-deps`) — pas seulement les graphes sidecar. Nécessite cache + gestion des gros graphes (sampling/LoD/incrémental). Synergie directe avec la vision IA/Model-as-graph. | 💡 Backlog |
