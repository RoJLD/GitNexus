...
| **P2** | **Boîte à outils théorie-des-graphes** | Centralités, communautés, chemins/cycles, points d'articulation, embeddings — exposés par graphe (endpoints + MCP + overlays). **Décomposé** en P2.1 (ci-dessous) + P2.2+. | 🚧 En cours |
| **P2.1** | **Centralité + communautés (v1)** | Module pur-JS zéro-dép sur la forme commune `{nodes,edges}` : degree + PageRank (power-iteration) + Louvain (modularité). Endpoint `GET /graph/metrics/:name` + outil MCP `gitnexus_graph_metrics` + overlay (taille=PageRank, couleur=communauté). Source = graphes **sidecar** (research/academic/research-graph). [spec](docs/superpowers/specs/2026-06-03-graph-platform-p2-1-graph-theory-design.md) | 📋 Spec écrite |
| **P2.2+** | **Reste de la boîte à outils (backlog)** | Centralités : betweenness, eigenvector, closeness, Katz, harmonic. Communautés : Leiden général, label-propagation, Louvain multi-niveaux + résolution, comparaison vs Leiden index-time de l'ASTKG. Chemins/structure : plus courts chemins, k-hop, détection de cycles, DAG/topo, **points d'articulation + ponts**, composantes connexes (faibles/fortes). Autres : k-core, coefficient de clustering/transitivité, assortativité, densité, diamètre/excentricité. **Embeddings** : node2vec/DeepWalk (lourd — possiblement hors pur-JS). Sources : **ASTKG** (via `/api/graph`), **lenses**, graphes **merged/group**. Surfaces : sélecteur de métrique + légendes, heatmaps de centralité, filtre/isolation de communauté, panneau top-N, export CSV/JSON, couverture MCP complète. Perf : **cache** des métriques (par graphe × algo, invalidé au re-ingest), **gros graphes** ASTKG (sampling/LoD/incrémental). Détail dans le spec P2.1 §6. Synergie : nourrit la vision IA/Model-as-graph (dead-weights/hot-paths) + P3. | 💡 Backlog |
