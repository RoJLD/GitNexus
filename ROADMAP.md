...
Dernière mise à jour : 2026-05-28 (Regression Forensics MVP Phase 1 livré (Tier 57) — 3e item de parité enterprise : GET /regression localise une régression entropy + commit coupable + fichiers (skeleton générique prêt pour Phase 2). Avant : Auto-reindexing (Tier 56), Code Wiki UI (Tier 55). 3/8 items enterprise couverts ou entamés.).
...
| **(Upcoming) Auto regression forensics** | 🟡 **MVP Phase 1 LIVRÉ (Tier 57)** — était 🔴. `GET /regression` localise une régression entropy (density/modularity) + classe le commit coupable + joint les fichiers, via l'attribution `/entropy/commits` (sans ML). | ✅ Phase 1 (entropy) : Tier 57. **Restants** : Phase 2 (ownership/dissonance/coupling sur le même skeleton), + "auto" (déclenchement sur franchissement de seuil watches → attacher le coupable au webhook), + UI highlight dans `EntropyCommitTimeline`. |
