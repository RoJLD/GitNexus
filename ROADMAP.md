...
Dernière mise à jour : 2026-05-27 (Roadmap-predictive Cleanup + Multi-tool connectors livré : `POST /ghosts/cleanup-prompt` retourne ghosts expirés + prompts LLM pré-construits par ghost ; `GET /ghosts/connector-suggestions` lit `.gitnexus.json > connectors.*`, fetch Plane open + closed tickets, fuzzy-matche par Jaccard, retourne suggestions reaffirm/cancel. Connectors framework avec Plane full v1 + Linear/GitHub/Jira stubs. `CleanupModal.tsx` ouvert via la 6ème card "Expired" de AuditSummary. v1 LLM call manuel — user copie le prompt).
