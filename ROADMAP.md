...
Dernière mise à jour : 2026-05-29 (Regression forensics polish livré (#62) — coupling 6e métrique watchable/auto-forensiquable + bouton "Locate regression" (bannière + ring) dans EntropyCommitTimeline ; clôt entièrement la ligne regression forensics. Aussi : Commit-level time-travel pièces A+B livrées — mode Commits timeline (#60) + baseline auto-seed caché/promote (#61) + pré-chauffage des diffs (#63) — chantier commit-level time-travel A+B+C COMPLET. Avant : "Auto" regression forensics livré (Tier 59) — webhooks watches enrichis du commit coupable au franchissement de seuil ; clôt l'item enterprise "Auto regression forensics" (on-demand Tiers 57-58 + auto). Avant : Regression Phase 2 (Tier 58), MVP (Tier 57), Auto-reindexing (Tier 56), Code Wiki UI (Tier 55). 3/8 items enterprise couverts (Code Wiki, Auto-reindexing, Regression forensics ✅).).
