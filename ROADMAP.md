...
| **P2** | **Boîte à outils théorie-des-graphes** | Centralités, communautés, chemins/cycles, points d'articulation, embeddings — exposés par graphe (endpoints + MCP + overlays). **Décomposé** en P2.1 + P2.2 + P2.3.1 + P2.3.2a (livrés) + P2.3.2b/c + P2.3.3 (backlog). | 🚧 P2.1·P2.2·P2.3.1·P2.3.2a livrés · P2.3.2b/c·P2.3.3 backlog |
...
| **P2.3.2b (B)** | **Collapse file-level complet de l'ASTKG** | Projection dédiée repliant **tous** les types de relations (pas juste `IMPORTS`) au niveau fichier → image plus riche de la structure du code, puis métriques. | 💡 Backlog |
| **P2.3.2c (B)** | **Niveau symbole + sampling/LoD + cache** | Métriques sur le graphe ASTKG brut au niveau symbole (fonctions/classes/fichiers — milliers de nœuds) : sampling/level-of-detail + cache de résultats + réglage du node-cap. Le cas lourd. | 💡 Backlog |
| **P2.3.3 (C)** | **Surfaces de visualisation des métriques** | Heatmap de coloration par centralité (continue), sélecteur de méthode de communauté dans l'overlay, rendu ponts/points-d'articulation, filtre/isolation de communauté, panneau top-N, export CSV/JSON. Rend lisibles les métriques déjà calculées. | 💡 Backlog |
