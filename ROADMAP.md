...
| **P3** | **Paradigmes de visualisation** | Décomposé en P3.1 (sélecteur de layout + layout hiérarchique) + P3.2 (matrice d'adjacence) + P3.3 (graphes research/lens en 3D + parité métriques) + P3.4 (nav multigraph). | 🚧 P3.1·P3.2·P3.3 livrés (+ QA navigateur) · P3.4 backlog |
...
| **P3.4** | **Nav multigraph (méta→graphe→nœud→inspecteur)** | Surface de navigation : niveau méta listant les graphes disponibles (nécessite un **nouvel endpoint serveur listant les instances sidecar scaffoldées** — aujourd'hui seuls les *templates* sont listables) → ouvrir un graphe → sélectionner un nœud → inspecteur. Réalise GraphSidebar Stage 2. Multi-part (serveur + frontend). | 💡 Backlog |
