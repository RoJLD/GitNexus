...
Dernière mise à jour : 2026-05-27 (Roadmap-predictive Gantt opérationnel livré : nouveau panneau `GanttPanel.tsx` calendaire des ghosts, axe X = temps, 4 styles de bar — solid matérialisé / dashed planifié futur / dot planifié sans deadline / grey annulé. Couleur par Tier + **time-decaying color** sur les dashed via `computeGhostVisualState` partagé avec Augmented graph (`late` → orange `#e67e22`, `critical` → rouge `#c0392b`). Toggle Swimlanes (flat vs Tier major), sort plannedAsc/tierAsc/status, export CSV client-side via Blob, filtres `ghostFilters?` réutilisant `passesFilter` + `DEFAULT_GHOST_FILTERS`. SVG natif, pure frontend, zéro endpoint serveur ajouté. Wiré dans `App.tsx` comme overlay toggle bottom-right au-dessus du bouton Audit.).
