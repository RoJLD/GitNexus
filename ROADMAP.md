...
Dernière mise à jour : 2026-05-28 (Regression Forensics Phase 2 livré (Tier 58) — /regression couvre 6 scalaires (entropy + ownership/dissonance/coupling en window-suspects) sur skeleton généralisé + git-utils partagé. Avant : Regression MVP Phase 1 (Tier 57), Auto-reindexing (Tier 56), Code Wiki UI (Tier 55). 3/8 items enterprise couverts/entamés.).
...
| **(Upcoming) Auto regression forensics** | 🟡 **Phases 1 + 2 LIVRÉES (Tiers 57-58)** — était 🔴. `GET /regression` couvre 6 scalaires (entropy density/modularity en attribution principielle ; ownership.busFactor/topAuthorShare + dissonance.purity + coupling en `window-suspects`). Sans ML. | ✅ Phase 1 (entropy, Tier 57) + Phase 2 (ownership/dissonance/coupling, Tier 58). **Restants** : "auto" (déclenchement sur franchissement de seuil watches → attacher le coupable au webhook), + UI highlight dans `EntropyCommitTimeline`. |
