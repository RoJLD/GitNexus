...
Dernière mise à jour : 2026-05-29 (Commit-level time-travel pièces A+B livrées — mode Commits timeline (#60) + baseline auto-seed caché/promote (#61), reste pièce C (pré-chauffage des diffs). Avant : "Auto" regression forensics livré (Tier 59) — webhooks watches enrichis du commit coupable au franchissement de seuil ; clôt l'item enterprise "Auto regression forensics" (on-demand Tiers 57-58 + auto). Avant : Regression Phase 2 (Tier 58), MVP (Tier 57), Auto-reindexing (Tier 56), Code Wiki UI (Tier 55). 3/8 items enterprise couverts (Code Wiki, Auto-reindexing, Regression forensics ✅).).
...
| **(Upcoming) Auto regression forensics** | ✅ **LIVRÉ (Tiers 57-59)** — était 🔴. On-demand `GET /regression` (6 scalaires, Tiers 57-58) **+ auto** : les webhooks watches sont enrichis du commit coupable au franchissement de seuil (Tier 59). Sans ML. | ✅ On-demand (Tiers 57-58) + auto (Tier 59). **Restant (optionnel)** : UI highlight dans `EntropyCommitTimeline` ; évaluateur watch `coupling` pour l'auto-forensics coupling. |
