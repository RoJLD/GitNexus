...
- ✅ 2bis.2 Commit-level entropy delta — [`upstream/docker-server-entropy-commits.mjs`](upstream/docker-server-entropy-commits.mjs) — `GET /entropy/commits?repo=&days=N` (ou `from/to` = SHA ou ISO). Attribue à chaque commit sa part proportionnelle (filesTouched) du delta entropy observé entre snapshots bracketants. Stragglers (hors-fenêtre snapshot) ressortent avec `attributedDensityDelta: null`. CSV export via `?format=csv`. MCP tool `gitnexus_entropy_commits`. Live test : hmm_studio sur 180j → 99 commits, 66 attribués, 33 stragglers, 4 windows.
