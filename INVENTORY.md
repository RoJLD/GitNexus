...
**Compteurs (autogénérés)** : MCP tools = <!-- COUNTER:mcp-tools -->39<!-- /COUNTER --> · additive-files = <!-- COUNTER:additive-files -->143<!-- /COUNTER --> · inplace-files = <!-- COUNTER:inplace-files -->25<!-- /COUNTER -->.
...
| [mcp-server/README.md](mcp-server/README.md) | Setup + protocole du sidecar MCP analytics (Tier 2bis.1) — <!-- COUNTER:mcp-tools -->39<!-- /COUNTER --> tools stdio JSON-RPC 2.0 zéro-dep, à brancher dans `~/.claude.json > mcpServers` |
...
- ✅ 2bis.1 MCP analytics sidecar — [`mcp-server/`](mcp-server/) — serveur stdio JSON-RPC 2.0 pure Node zéro-dep, 13 tools à la livraison 2bis.1 (12 endpoints + `gitnexus_repo_by_id`) → <!-- COUNTER:mcp-tools -->39<!-- /COUNTER --> tools aujourd'hui (analytics + copilot + lens + query_meta_graph). Coexiste avec `npx gitnexus mcp` upstream (pas de patch dans `upstream/`). Smoke ✓ (`mcp-server/smoke.mjs`).
