...
**Snapshot daté : 2026-06-15 (post-Tier-3.7 Phase D Hardening scaffold)** — entête **réconciliée 2026-08-19** (bump v1.6.7 → v1.6.9 EXÉCUTÉ)

> ✅ **Bump v1.6.7 → v1.6.9 (2026-08-19)** — **sources+patches = image CLI = `v1.6.9`**, en UNE passe depuis UN seul clone `upstream/`. 25 fichiers in-place → 14 clean / 11 conflits résolus à la main ; `package-lock.json` jamais mergé à la main (régénéré via `npm install --package-lock-only`). Deux ruptures silencieuses SANS marqueur de conflit, trouvées par `tsc -b` sur l'arbre fusionné : l'import `createKnowledgeGraph` supprimé par l'amont alors que 7 sites du fork en dépendent (restauré), et la collision `graphMode` amont `'full'|'chatOnly'` (#2178) contre `'single'|'diff'` du fork (**c'est le côté fork qui a été renommé** en `timelineGraphMode` — renommer l'amont se re-paierait à chaque bump). Gates à froid : `tsc -b` 0, `vite build` ok, `vitest` 361/361, `check-patch-drift` 0, `check-doc-counters` 0.

**Base upstream (sources+patches) : `v1.6.9`** (bump exécuté 2026-08-19, aligné sur l'image CLI ; dernier tag stable amont — aucun `chore: release` entre v1.6.9 et `origin/main`).
...
| `GET /churn` | Heatmap de volatilité des nodes sur la timeline. **Cache partagé per-snapshot (2026-07-15, ↓)** |
| `GET /coupling` | Paires de fichiers qui changent ensemble (couplage temporel). **Cache partagé per-snapshot (↓)** |
...
- `hooks/useTimelineUrlSync.ts` + `lib/timeline-url.ts` — **Timeline URL Persistence Phase 2 Item #5** (Tier 53) : sync bidirectionnel état Timeline ↔ URL via 5 params `tl*` (tlA/tlB/tlZoom/tlMode/tlFilter), shortHash-based (`live` alias pour le head, stable across re-index). `lib/timeline-url.ts` = 2 pure fns testées (`serializeTimelineToParams` → `{set, remove}` clean-URL, `parseTimelineParams` → `TimelineUrlState`). Le hook (monté dans `AppContent` de `App.tsx`) fait : READ one-shot guardé (`readDone` ref) attendant `repo.snapshots?.length`, puis WRITE `replaceState` sur changement. Zoom restauré en différé (`pendingZoom` ref → effect qui fire `enterZoom()` quand cursors flushés). Aligné sur le pattern `?project=`/`?server=` existant. **FIX 2026-07-12 (bug prod)** : ce hook ET le diff effect de `useAppState` résolvaient cursor↔snapshot via `availableRepos[repo].snapshots` — un champ que `/api/repos` **ne remplit jamais** (mesuré) → le cursor-diff ne pouvait diffuser QUE la tête live, jamais 2 snapshots distincts, **pour de vrais users**. Fix : `useAppState` fetch `/snapshots` du repo actif et enrichit `availableRepos[repo].snapshots` (source unique ; muter re-déclenche les 2 consommateurs ; garde idempotente `length !==`). Prouvé live (légende rendue avec comptes réels). Cf spec cursors § Update 2026-07-12 (suite).
- **Harnais e2e (2026-07-12)** — `tests/e2e/playwright.config.ts` (**était ABSENT** → le tier ne s'exécutait jamais, masqué par le job CI `continue-on-error`) : testDir `specs`, baseURL `E2E_WEB_URL`, chromium, reporter → `tests/playwright-report`. `tests/e2e/helpers/connect.ts` (`connectRepo` via `?project=&server=`, E2E_REPO/E2E_API_URL). Spec `timeline-zoom-and-diff` migré : 8/9 verts (le 9ᵉ = flake molette), **test légende cursor-diff DÉTERMINISTE** (découvre 2 shortHashes via `/snapshots`→tlA/tlB/tlMode=diff→asserte only-in-A/B/both). Reste : migrer les ~13 autres specs vers `connectRepo`, puis retirer `continue-on-error`.
...
- **Formulaire guidé (2ᵉ surface d'entrée, 2026-07-19)** : traducteur pur `upstream/gitnexus-web/src/lib/lens-form-spec.ts` (`formStateToSpec` — omet un bloc plutôt que de l'émettre vide : `color` sans `by` casse la grammaire backend E6, un `select`/`insight` vide n'a pas de sens ; un seul prédicat émis, `eval_predicate` n'a pas de conjonction) + composant **additif** `GuidedLensForm.tsx` (champs contrôlés `{state,onChange}` : name/source/meaning/predicat/color/insight). Toggle `guided|expert` dans `LensStudioModal` (les boutons Preview/Propose restent **partagés** — `parse()` devient conscient du mode, en guidé la spec vient toujours du formulaire, jamais d'erreur JSON) ; basculer guidé→expert **sérialise** la spec construite dans le textarea (`JSON.stringify(formStateToSpec(form), null, 2)` — preuve visible que les deux surfaces émettent le même objet canonique) ; basculer expert→guidé ne tente **aucun reverse-parsing** (une spec JSON arbitraire n'est pas toujours représentable dans le formulaire ; prétendre le contraire droppererait silencieusement des champs) — mais la sérialisation guidé→expert est **conditionnée à `isFormPristine`** : la direction seule n'est pas un consentement, un aller-retour expert→guidé→expert sans une seule frappe écrasait sinon le JSON écrit à la main par `{"name":"","source":{"lens":""}}` (revue finale, sans avertissement ni undo). **Garde cliente `validateFormState`** (revue finale) — le guidé n'en avait aucune : `parse()` retournait toujours une spec, donc `if (!spec) return;` ne se déclenchait jamais et un formulaire vierge POSTait réellement au canon souverain. Elle exige `name` + `source.lens` (un amont vide est *falsy* → le garde d'intégrité référentielle du promoteur est court-circuité et l'intention de composition part à la poubelle en silence), rejette tout opérande de prédicat portant `< > = !` (la grammaire backend scanne les opérateurs par *containment* avant d'atteindre sa branche ` in ` → `path in [a>b, c]` partitionne sur `>`, tous les nœuds évaluent False, graphe entièrement éteint sans erreur) et remonte un `insight` mal formé au lieu de l'omettre (`min={1}` n'est jamais appliqué — aucun `<form>`, boutons `type="button"` ; `top 0/-3/2.5 by x` et `top 5 by in-degree` rendent un panneau insights vide, vert, inexpliqué). Codes d'erreur i18n purs (`lensStudio.guided.errors.*` en/zh-CN) — le traducteur reste React-free et locale-free. A11y : `title` sur les 2 `<select>` (aucun n'est enfant d'un `<label>`), `aria-pressed` sur le segmented control. 72 unit (44 pures + 12 formulaire + 16 modal), 861 unit verts sur la suite complète ; les 4 mutations restées vertes en revue finale sont désormais **tuées** (12 mutations rejouées, 12 rouges). [spec](docs/superpowers/specs/2026-07-19-guided-lens-form-design.md) | `upstream/gitnexus-web/src/lib/lens-form-spec.ts` (additif), `upstream/gitnexus-web/src/components/GuidedLensForm.tsx` (additif), `upstream/gitnexus-web/src/components/LensStudioModal.tsx` (édit), `upstream/gitnexus-web/src/locales/{en,zh-CN}/header.json`, `tests/unit/lens-form-spec.test.ts`, `tests/unit/components/guided-lens-form.test.tsx`, `tests/unit/components/lens-studio-modal.test.tsx`
...
| [mcp-server/README.md](mcp-server/README.md) | Setup + protocole du sidecar MCP analytics (Tier 2bis.1) — <!-- COUNTER:mcp-tools -->37<!-- /COUNTER --> tools stdio JSON-RPC 2.0 zéro-dep, à brancher dans `~/.claude.json > mcpServers` |
...
- ✅ 2bis.1 MCP analytics sidecar — [`mcp-server/`](mcp-server/) — serveur stdio JSON-RPC 2.0 pure Node zéro-dep, 13 tools à la livraison 2bis.1 (12 endpoints + `gitnexus_repo_by_id`) → <!-- COUNTER:mcp-tools -->37<!-- /COUNTER --> tools aujourd'hui (analytics + copilot + lens + query_meta_graph). Coexiste avec `npx gitnexus mcp` upstream (pas de patch dans `upstream/`). Smoke ✓ (`mcp-server/smoke.mjs`).
- ✅ 2bis.1.lens **Les 3 surfaces du contrat `/lens` exposées à l'agent** — `gitnexus_list_lenses` (`/lens`, découverte), `gitnexus_get_lens_graph` (`/lens/<name>`, traversée), et **`gitnexus_narrate_lens`** (`/lens/<name>/narrate`, lecture — livré 2026-07-21, Déjà livré #74). La narration était le consommateur prévu au design de la gateway et jamais câblé ; l'agent recevait la lentille entière (~1500 nœuds, la population dérive à chaque régénération) pour une question à laquelle un brief markdown répondait déjà. Mesuré sur la lentille `sigil` (2026-07-21, **octets UTF-8 du bloc texte MCP tel que délivré** — la base que le contexte de l'agent paie réellement) : **~43 kB contre ~1,7 MB, ~40×**. Le smoke asserte la **direction**, pas la constante. **Pas d'option `synthesis`** : la route HTTP la garde pour un humain, mais elle n'est pas exposée au MCP — mesuré, `_narration_synthesis()` demande le tier `claude-sonnet` mais le routage budgétaire SIGIL-529 (aucune clé Anthropic configurée) l'exécute en local, `ollama/deepseek-r1:8b` / `local-fast` / `cost_eur=0.0` ; quatre échantillons de latence (31,3 s · 42,5 s · 43,8 s · 47,1 s, plus un dépassement au-delà de 120 s) rendent tout plafond indéfendable ; et le rendement est erratique — 45, 450, 474 puis 31 octets sur quatre exécutions de la MÊME requête, soit une dispersion de 15×. Ce qui est stable, c'est l'échec : **les quatre sont tronqués en pleine phrase** (le modèle local est un modèle de raisonnement, ses tokens de réflexion épuisent le budget de 400 avant la réponse). Une révision antérieure de cette ligne publiait « 45 octets pour 42,5 s » comme LE rendement : c'était un tirage unique près du plancher, sous-estimant d'environ 10× — la décision survit à la correction, le chiffre non. Les trois outils dégradent en stub documenté sans `INTER_GRAPH_URL` (Zero Masking) ; lentille inconnue → erreur, pas résultat vide. **Note** : `KNOWN_MCP_TOOLS` dans [`upstream/docker-server-copilot-core.mjs`](upstream/docker-server-copilot-core.mjs) reste à 34 et n'inclut aucun des 3 outils lens. Son commentaire d'en-tête le déclare « Static mirror of the MCP tool names registered in `mcp-server/server.mjs` » — c'est donc bien un miroir du registre MCP, et il est **périmé** (il annonce aussi un smoke qui assertait 34, alors qu'il asserte 37). La divergence est antérieure (les 2 outils lens précédents y manquaient déjà) et **non corrigée ici pour une seule raison** : le fichier vit dans `upstream/`, hors périmètre, et l'éditer imposerait une régénération des patches. Le gate reste `GREEN` car il porte sur les 9 endpoints de `REQUIRED_ENDPOINTS`, qui sont un sous-ensemble distinct.
