...
| Guided lens form spec — pure translator | `unit/lens-form-spec.test.ts` | `predicateRowToExpr` comparison ops + bracketed `in` list ; `formStateToSpec` minimal spec, OMITS `color`/`select`/`insight` blocks when unset (empty block breaks backend grammar), emits them when set, omits blank `meaning` — 11 cases |
