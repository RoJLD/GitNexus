...
| Lens — inLens dim (browser projector) | `unit/lens-inlens-dim.test.mjs` | `knowledgeGraphToGraphology` dims `properties.inLens===false` nodes (color `#374151` + shrunk size), leaves `inLens:true`/absent nodes intact — 3 cases |
...
| Lens authoring client — preview/propose | `unit/lens-authoring-client.test.ts` | `previewLens` 200 → `{ok,braingraph}` / 422 → `{ok:false,errors}` / transport throw → `{ok:false}` (never rejects) ; `proposeLens` sends `Authorization: Bearer` + body `{spec,auto_approve}` WITHOUT `author` (server-derived) + FastAPI `{detail}` surfaced ; `setBridgeUrl` reuses the `validateBackendUrl` SSRF scheme-guard — 6 cases |
| Lens preview render — useAppState.previewLens | `unit/use-app-state-preview-lens.test.tsx` | ok path builds the graph then applies lens metadata (`lensInsights`/`lensMeaning` set — the gap-#7 regression class) ; non-ok returns `{ok:false,errors}` without replacing the graph — 2 cases |
| Guided lens form spec — pure translator + client gate | `unit/lens-form-spec.test.ts` | `predicateRowToExpr` comparison ops + bracketed `in` list ; `formStateToSpec` minimal spec, OMITS `color`/`select`/`insight` blocks when unset (empty block breaks backend grammar), emits them when set, omits blank `meaning`, never emits an `insight.rank` the backend answers with `[]` and no error (`top 0/-3/2.5 by x`, `top 5 by in-degree` — `min={1}` is never enforced, no `<form>` wraps the fields) ; `validateFormState` issue codes — name/`source.lens` required (an empty upstream is falsy, so the promoter's referential-integrity guard is short-circuited), predicate operand carrying `< > = !` rejected (eval_predicate scans operators by containment before its ` in ` branch → whole graph silently dimmed), half-filled/out-of-range insight reported instead of dropped, fully-blank insight treated as an opt-out ; `isFormPristine` per-field (guards the guided→expert serialization against destroying hand-written expert JSON) — 44 cases |
...
| LensStudioModal | `unit/components/lens-studio-modal.test.tsx` | authoring modal — `null` when closed, textarea JSON parsed then `onPreview(spec)` called with the canonical spec, invalid JSON shows an error and does NOT call `onPreview`, success toast on `{ok:true}` Preview, `onPropose(spec, autoApprove)` receives the checkbox state, guided/expert mode toggle: in guided mode Preview receives the spec BUILT BY THE FORM (`formStateToSpec`), switching guided→expert serializes the built spec into the textarea (isomorphism proof, no reverse-parsing expert→guided), switching expert→guided clears a stale JSON error/toast from the other surface (review fix — a parse error is impossible in guided mode) ; final-review fixes: guided Propose (the only action with a persistent side effect) receives the form-built spec, an empty guided form shows errors and calls NEITHER `onPropose` nor `onPreview`, a pristine expert→guided→expert round-trip leaves hand-written JSON byte-intact while an edited one still serializes (isomorphism preserved), errors/toast cleared in BOTH switch directions, `aria-pressed` on the segmented control — 16 cases (react-i18next + lucide mocked file-scoped: `tests/` and `upstream/gitnexus-web/` are separate npm installs, rendering the real i18n hook throws "Invalid hook call") |
| GuidedLensForm | `unit/components/guided-lens-form.test.tsx` | 2nd input surface for Lens Studio — controlled `{state,onChange}` form fields (name/source/meaning/predicate row/color/insight) report edits through `onChange` with the patched `LensFormState` — every one of the 10 controls now pins the exact state key it patches (a control wired to the wrong key was invisible to the 3-control original), a badInput-blanked number input reports `''` not `NaN`, both `<select>`s carry an accessible name — 12 cases (same react-i18next + lucide mock pattern as `lens-studio-modal.test.tsx`) |
